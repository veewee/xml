...
final class NodeListTest extends TestCase
{
    ...
    public function test_it_can_be_iterated(): void
    {
        ...
        foreach ($items as $item) {
            static::assertInstanceOf(Element::class, $item);
        }
    }
    ...
    public function test_it_can_map(): void
    {
        $items = $this->loadProducts()->map(
            static fn (Element $element): string => $element->textContent
        );
    ...
    public function test_it_can_loop_over_all_items(): void
    {
        ...
        $this->loadProducts()->forEach(
            static function (Element $element) use ($x) : void {
    ...
    public function test_it_can_filter(): void
    {
        ...
        $filtered = $all->filter(
            static fn (Element $element): bool => (bool) ((int)($element->getAttribute('id'))%2)
        );
    ...
    public function test_it_can_reduce(): void
    {
        $total = $this->loadPrices()->reduce(
            static fn (int $total, Element $element): int => $total + (int) $element->textContent,
            0
        );
    ...
    public function test_it_can_detect(): void
    {
        $list = $this->root()->detect(
            static fn (Element $element) => filter(
                $element->childNodes,
                static fn (Node $current) => is_element($current)
            )
        );
    ...
    public function test_it_can_expect_single_on_a_two_item_list(): void
    {
        ...
        $prices->filter(static fn (Element $price): bool => (int)$price->textContent >= 2)->expectSingle('Expected single element, got %s elements.');
    }
    ...
    {
        $prices = $this->loadPrices();
        $result = $prices->expectAllOfType(Element::class);
    ...
    public function test_it_can_validate_types_and_fail(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $prices = $this->loadPrices();
        $prices->expectAllOfType(Attr::class);
    }

    public function test_it_can_sort(): void
    {
        $prices = $this->loadPrices();
        $sorted = $prices->sort(static fn (Node $a, Node $b) => $b->textContent <=> $a->textContent);
